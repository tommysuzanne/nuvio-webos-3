...
if (arg && arg!=='--check-only') {
  const result=spawnSync(process.execPath,[path.join(root,'scripts/build-uj630.mjs'),path.resolve(arg)],{cwd:root,stdio:'inherit',env:{...process.env,NUVIO_UI_RESOLUTION:'1920x1080',NUVIO_BUILD_LABEL:process.env.NUVIO_BUILD_LABEL||'uj630-public.43'}});
...
try {
  const result=spawnSync('npm',['run','package:webos'],{cwd:root,stdio:'inherit',env:{...process.env,NUVIO_FIXED_VIEWPORT:'1',NUVIO_UI_SCALE:'0.8',NUVIO_UI_RESOLUTION:'1920x1080',NUVIO_BUILD_LABEL:temporary?'uj630-public.CHECK-ONLY':process.env.NUVIO_BUILD_LABEL||'uj630-public.43',NUVIO_REQUIRE_LOCAL_PROPERTIES:'1'}});
  if(result.error)throw result.error;
