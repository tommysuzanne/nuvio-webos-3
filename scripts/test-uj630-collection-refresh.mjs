...
now+=60001;const failed=service.request();responses.shift()(syncResult('error'));assert.equal((await failed).ok,false);assert.equal(calls.length,3,'organization not read after invalid collections');
assert(success.get('1collections')<now);const retry=service.request();responses.shift()(syncResult('unchanged'));await retry;assert.equal(calls.length,5,'error does not reset one-minute success clock');
...
console.log('PASS targeted refresh: 1min success freshness, full/manual coalescing, one publication, failure retry eligibility, session/profile separation.');
