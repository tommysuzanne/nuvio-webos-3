...
Removed from publication: account backups, local settings, personally configured IPKs, personal API keys and addon URLs, collection/profile exports, private TV diagnostics, personal artwork and its URL mapping, developer pairing keys, machine addresses, build caches and dependencies. The preparation tool remains available for personal use.
...
CI builds only a placeholder package and never connects to a TV. Separately reviewed releases may include a compiled package using only existing public upstream client configuration. The public release helper pins and verifies its provenance, refuses local personal settings/artwork and produces a checksum. No automatic upstream promotion or publication is enabled. See [INSTALLATION](INSTALLATION.md).
